...
def count_empty_and_friends(friends: list, x: int, y: int):
    empty, friend = 0, 0
    for i in range(4):
        nx = x + dx[i]
        ny = y + dy[i]
...
for favorite in table:
        ...
        for i in range(N):
            for j in range(N):
                if seats[i][j] is None:
                    empty, friend = count_empty_and_friends(seated_favorite, j, i)
                    if not max_count:
    ...
    # 좋아하는 친구가 아직 아무도 못앉았을 때
        ...
        for i in range(N):
            for j in range(N):
                if seats[i][j] is None:
                    empty, friend = count_empty_and_friends([], j, i)
                    if not max_count:
                        max_count = [empty, i, j]
...
for i in range(N):
    for j in range(N):
        ...
        empty, friend = count_empty_and_friends(favorite, j, i)
        if friend == 0:
            score += 0
        elif friend == 1:
            score += 1
...
show_list('table', table)
show_list('seats', seats)
show_dic('seated', seated)
'''